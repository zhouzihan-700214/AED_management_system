...
def address_contains_postal_code(address: Any, postal_code: Any) -> bool:
    ...
    if not postal or not address_text:
        return False
    return bool(re.search(f'(?<!\\d){re.escape(postal)}(?!\\d)', address_text))
...
def coordinates_are_current(row: Any) -> bool:
    ...
    return is_valid_number(latitude) and is_valid_number(longitude) and (status == 'Success') and address_contains_postal_code(row.get('OneMap Address', ''), row.get('Postal Code', ''))

def update_missing_coordinates(csv_file: str | Path=AED_DATA_FILE, *, create_backup: bool=False, pause_seconds: float=0.2) -> dict[str, Any]:
    """
    ...
    summary: dict[str, Any] = {'total': len(dataframe), 'missing_or_stale_before': len(refresh_indexes), 'updated': 0, 'failed': 0, 'skipped': len(dataframe) - len(refresh_indexes), 'backup_path': ''}
    if not refresh_indexes:
        return summary
    if create_backup:
        backup_name = f"aed_data_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    ...
    for row_index in refresh_indexes:
        ...
        if result.success:
            summary['updated'] += 1
            print(f'[SUCCESS] {serial_number}: {result.latitude}, {result.longitude}')
        else:
            summary['failed'] += 1
...
def main() -> None:
    summary = update_missing_coordinates(AED_DATA_FILE, create_backup=True)
    print('\nFinished.')
    print(f"Updated coordinates: {summary['updated']}")
    print(f"Already current: {summary['skipped']}")
    print(f"Failed: {summary['failed']}")
    if summary['backup_path']:
        print(f"Backup created: {Path(summary['backup_path']).name}")
