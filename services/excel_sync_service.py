...
import pandas as pd
from config import AED_CACHE_FILE, CACHE_BACKUP_DIR, EXCEL_FILE, EXCEL_SHEET, MAX_CACHE_BACKUPS, PRESERVE_CACHE_ONLY_UNITS, SERIAL_COLUMN, SYNC_LOCK_FILE, SYNC_STATE_FILE, TEMP_DIR, ensure_project_directories
from services.aed_service import DATE_COLUMNS, MASTER_COLUMNS, ensure_columns, load_aed_data, save_aed_data
from services.excel_lock_service import operation_lock
from services.column_mapping import clean_excel_header, map_excel_header, normalise_header
...
CACHE_OWNED_COLUMNS = ['Latitude', 'Longitude', 'OneMap Address', 'Geocoding Status', 'PM Interval Months']
REAL_IB_REQUIRED_HEADERS = {'Serial Number', 'Block / Locations', 'Street Name', 'Postal Code', 'Next PM Due'}
...
def save_sync_state(state: dict[str, Any], state_file: str | Path=SYNC_STATE_FILE) -> None:
    path = Path(state_file)
    path.parent.mkdir(parents=True, exist_ok=True)
    temporary_path: Path | None = None
    try:
        with tempfile.NamedTemporaryFile(mode='w', suffix='.json', prefix=f'.{path.stem}_', dir=path.parent, delete=False, encoding='utf-8') as handle:
...
def _record_result(result: SyncResult, *, signature: ExcelSignature | None, state_file: str | Path, excel_file: str | Path, excel_sheet: str) -> None:
    previous = load_sync_state(state_file)
    state = {**previous, 'excel_file': str(Path(excel_file)), 'excel_sheet': excel_sheet, 'observed_excel_signature': _signature_dict(signature), 'last_checked_at': _now_text(), 'last_sync_time': result.synced_at or previous.get('last_sync_time', ''), 'sync_status': result.status, 'sync_message': result.message, 'row_count': result.row_count, 'warnings': list(result.warnings)}
...
def _copy_stable_snapshot(source: Path, temp_dir: Path) -> tuple[Path, ExcelSignature]:
    ...
    for _ in range(3):
        ...
        snapshot = temp_dir / f"read_snapshot_{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}{source.suffix}"
        shutil.copy2(source, snapshot)
        after = get_excel_signature(source)
        if before == after:
            return (snapshot, after)
        snapshot.unlink(missing_ok=True)
    raise OSError('The Excel file changed while it was being copied. Save the workbook completely, then refresh again.')

def _normalise_lock_path(lock_file: str | Path, state_file: str | Path) -> Path:
...
def _acquire_sync_lock(lock_file: Path, stale_seconds: int=300) -> Iterator[None]:
    del stale_seconds
    metadata = {'operation_id': f"refresh-{datetime.now().strftime('%Y%m%d%H%M%S%f')}", 'operation_type': 'REFRESH_EXCEL', 'user': 'System', 'session_id': 'manual-refresh', 'serial_numbers': []}
...
def _format_excel_date(value: Any) -> str:
    ...
    try:
        ...
        if numeric_value.is_integer() and 20000 <= numeric_value <= 80000:
            parsed = pd.Timestamp('1899-12-30') + pd.to_timedelta(int(numeric_value), unit='D')
            return parsed.strftime('%d-%m-%Y')
    except (TypeError, ValueError):
        pass
    if re.match('^\\d{4}[-/]\\d{1,2}[-/]\\d{1,2}(?:\\D|$)', text):
...
def _format_battery_history(value: Any) -> str:
    ...
    if ',' in text or ';' in text or ' / ' in text:
        return text
    if re.match('^\\d{4}[-/]\\d{1,2}[-/]\\d{1,2}(?:\\s|$)', text):
        parsed = pd.to_datetime(text, errors='coerce', yearfirst=True)
...
def _header_lookup(columns: list[object]) -> dict[str, str]:
    return {normalise_header(column): clean_excel_header(column) for column in columns}
...
def _is_real_ib_layout(raw: pd.DataFrame) -> bool:
    ...
    return {normalise_header('SERIAL NUMBER'), normalise_header('Block / Locations'), normalise_header('Street Name')}.issubset(headers)

def _validate_real_ib_headers(raw: pd.DataFrame) -> None:
    lookup = _header_lookup(list(raw.columns))
    missing = [header for header in sorted(REAL_IB_REQUIRED_HEADERS) if normalise_header(header) not in lookup]
    if missing:
        raise ValueError('The Excel sheet is missing required column(s): ' + ', '.join(missing))
...
def _prepare_real_ib_rows(raw: pd.DataFrame) -> tuple[pd.DataFrame, list[str]]:
    ...
    for index, row in source.iterrows():
        ...
        if serial:
            ...
            continue
        continuation = _clean_scalar(row.get(remarks_column, '')) if remarks_column is not None else ''
        if continuation and records and (remarks_column is not None):
            records[-1][remarks_column] = _combine_remarks(records[-1].get(remarks_column, ''), continuation)
            serial_value = _clean_scalar(records[-1].get(serial_column, ''))
            warnings.append(f'Excel row {excel_row} was merged into {serial_value} Remarks.')
    if not records:
...
def _normalise_excel_dataframe(raw: pd.DataFrame) -> tuple[pd.DataFrame, tuple[str, ...]]:
    warnings: list[str] = []
    dataframe = raw.copy()
    if _is_real_ib_layout(dataframe):
        ...
        dataframe['Location'] = dataframe.apply(lambda row: _build_location(row.get(block_column, '') if block_column else '', row.get(street_column, '') if street_column else ''), axis=1)
    ...
    dataframe['Battery Replacement History'] = dataframe['Battery Replacement History'].map(_format_battery_history)
    ...
    intervals = pd.to_numeric(dataframe['PM Interval Months'], errors='coerce').fillna(12).clip(lower=1, upper=60).astype(int)
    ...
    for index, row in dataframe.iterrows():
        ...
        for column in DATE_COLUMNS:
            ...
            if not pd.isna(parsed) and (not 2000 <= int(parsed.year) <= 2100):
                warnings.append(f'{serial} {column} looks unusual ({value}). Check the source Excel cell.')
...
def _validate_excel_dataframe(dataframe: pd.DataFrame) -> None:
    ...
    if duplicate_mask.any():
        ...
        raise ValueError('Duplicate Serial Number values were found: ' + ', '.join(duplicates[:10]))

def _serial_key(value: Any) -> str:
    return _clean_scalar(value).casefold()

def _merge_excel_with_cache(excel_dataframe: pd.DataFrame, cache_file: Path, *, preserve_cache_only_units: bool) -> tuple[pd.DataFrame, tuple[str, ...]]:
    ...
    try:
        ...
        return (ensure_columns(excel_dataframe), ('The previous CSV mirror could not be read, so website-only fields were not preserved.',))
    result = ensure_columns(excel_dataframe).copy()
    cache = ensure_columns(cache)
    cache_lookup = {_serial_key(row[SERIAL_COLUMN]): row for _, row in cache.iterrows() if _serial_key(row[SERIAL_COLUMN])}
    warnings: list[str] = []
    ...
    for index, row in result.iterrows():
        ...
        if old_postal == new_postal:
            for column in ['Latitude', 'Longitude', 'OneMap Address', 'Geocoding Status']:
                result.at[index, column] = _clean_scalar(existing.get(column, ''))
        else:
            ...
            warnings.append(f'{row[SERIAL_COLUMN]} Postal Code changed; coordinates were cleared for automatic refresh.')
    cache_only = cache[~cache[SERIAL_COLUMN].map(_serial_key).isin(excel_keys)].copy()
    if preserve_cache_only_units and (not cache_only.empty):
        result = pd.concat([result, cache_only], ignore_index=True)
        warnings.append(f'Retained {len(cache_only)} existing website unit(s) not present in the test Excel because PRESERVE_CACHE_ONLY_UNITS is enabled.')
...
def _backup_cache(cache_file: Path, backup_dir: Path, max_backups: int) -> None:
    ...
    backup_path = backup_dir / f"aed_data_{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}.csv"
    shutil.copy2(cache_file, backup_path)
    backups = sorted(backup_dir.glob('aed_data_*.csv'), key=lambda path: path.stat().st_mtime_ns, reverse=True)
    for old_backup in backups[max(1, int(max_backups)):]:
        old_backup.unlink(missing_ok=True)

def sync_excel_to_cache(*, force: bool=False, excel_file: str | Path=EXCEL_FILE, excel_sheet: str=EXCEL_SHEET, cache_file: str | Path=AED_CACHE_FILE, state_file: str | Path=SYNC_STATE_FILE, temp_dir: str | Path=TEMP_DIR, lock_file: str | Path=SYNC_LOCK_FILE, backup_dir: str | Path=CACHE_BACKUP_DIR, preserve_cache_only_units: bool=PRESERVE_CACHE_ONLY_UNITS, max_backups: int=MAX_CACHE_BACKUPS, acquire_lock: bool=True, lock_metadata: dict[str, Any] | None=None) -> SyncResult:
    ...
    if signature is None:
        result = SyncResult(status='csv_fallback', message=f'{source.name} is not present. The website is using {cache.name} as a temporary local source.', source_exists=False, changed=False, row_count=_safe_cache_row_count(cache))
        previous = load_sync_state(state_file)
        if previous.get('sync_status') != 'csv_fallback' or previous.get('row_count') != result.row_count:
            _record_result(result, signature=None, state_file=state_file, excel_file=source, excel_sheet=excel_sheet)
        return result
    state = load_sync_state(state_file)
    if not force and state.get('last_successful_signature') == _signature_dict(signature) and cache.exists() and (cache.stat().st_size > 0):
        return SyncResult(status='up_to_date', message='The website CSV mirror already matches the latest Excel version.', source_exists=True, changed=False, row_count=_safe_cache_row_count(cache), synced_at=str(state.get('last_sync_time', '')), warnings=tuple(state.get('warnings', []) or []))
    snapshot: Path | None = None
    resolved_lock = _normalise_lock_path(lock_file, state_file)
    try:
        lock_context = _acquire_sync_lock(resolved_lock) if acquire_lock else nullcontext()
        with lock_context:
            signature = get_excel_signature(source)
            state = load_sync_state(state_file)
            if not force and signature is not None and (state.get('last_successful_signature') == _signature_dict(signature)) and cache.exists() and (cache.stat().st_size > 0):
                return SyncResult(status='up_to_date', message='The website CSV mirror already matches the latest Excel version.', source_exists=True, changed=False, row_count=_safe_cache_row_count(cache), synced_at=str(state.get('last_sync_time', '')), warnings=tuple(state.get('warnings', []) or []))
            snapshot, stable_signature = _copy_stable_snapshot(source, Path(temp_dir))
            try:
                raw = pd.read_excel(snapshot, sheet_name=excel_sheet, dtype=object, header=0)
            except ValueError as error:
                with pd.ExcelFile(snapshot) as workbook:
                    available = ', '.join(workbook.sheet_names)
                raise ValueError(f"Worksheet '{excel_sheet}' was not found. Available sheets: {available}.") from error
            dataframe, parse_warnings = _normalise_excel_dataframe(raw)
            _validate_excel_dataframe(dataframe)
            dataframe, merge_warnings = _merge_excel_with_cache(dataframe, cache, preserve_cache_only_units=preserve_cache_only_units)
            _validate_excel_dataframe(dataframe)
            ...
            result = SyncResult(status='synced', message=f'Imported {len(dataframe)} AED unit(s) from {source.name} [{excel_sheet}].', source_exists=True, changed=True, row_count=len(dataframe), synced_at=synced_at, warnings=warnings)
            _record_result(result, signature=stable_signature, state_file=state_file, excel_file=source, excel_sheet=excel_sheet)
            return result
    except Exception as error:
        result = SyncResult(status='failed', message=str(error), source_exists=True, changed=False, row_count=_safe_cache_row_count(cache))
        _record_result(result, signature=signature, state_file=state_file, excel_file=source, excel_sheet=excel_sheet)
        raise
    finally:
