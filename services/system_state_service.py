...
from config import MICROSOFT_CONFIG, ONEDRIVE_CLOUD_ENABLED, PROJECT_ROOT, SYSTEM_STATE_PATHS, SYSTEM_STATE_PENDING_DIR, SYSTEM_STATE_SYNC_FILE
from services.onedrive_file_service import OneDriveFileConflictError, OneDriveFileError, download_bytes, get_metadata, upload_bytes
...
def _remote_path() -> str:
    return str(MICROSOFT_CONFIG.get('system_state_path', '/AED System/AED_System_State.zip') or '/AED System/AED_System_State.zip').strip()
...
def bootstrap_system_state() -> SystemStateResult:
    ...
    if remote is None:
        ...
        _write_state({'remote_path': _remote_path(), 'etag': uploaded.etag, 'local_fingerprint': current_fingerprint, 'last_sync_time': now})
        return SystemStateResult('initialised', 'A separate OneDrive system-state archive was created.', changed=True, uploaded=True)
    state = _read_state()
    ...
    if not state or state.get('etag') != remote.etag:
        ...
        _write_state({'remote_path': _remote_path(), 'etag': downloaded.etag, 'local_fingerprint': fingerprint, 'last_sync_time': now})
        return SystemStateResult('downloaded', 'Latest system records were loaded from OneDrive.', changed=True, downloaded=True)
    return SystemStateResult('up_to_date', 'System records are up to date.')
...
def sync_system_state(*, allow_download: bool=True) -> SystemStateResult:
    ...
    if local_changed and remote_changed:
        ...
        return SystemStateResult('conflict', f'System records changed in both places. A local recovery archive was saved at {pending}. Reload before making more changes.')
    if remote_changed and (not allow_download):
        return SystemStateResult('deferred', 'A newer system-state archive is available. It will be loaded after the current editing workspace is closed.')
    if remote_changed:
        content, downloaded = download_bytes(_remote_path())
        _safe_extract(content)
        fingerprint = local_fingerprint()
    ...
    if local_changed:
        try:
            uploaded = upload_bytes(_remote_path(), build_archive(), content_type='application/zip', expected_etag=previous_etag)
...
# Signature contract: allow_download: bool = True
