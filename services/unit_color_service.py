...
ROLE_FALLBACK_NAMES = {'Pending': 'Pending', 'Completed': 'Completed', 'Issue': 'Issue', 'Pending Verification': 'Pending Verification', 'Out of Service': 'Out of Service'}
...
def _read_csv(path: str | Path) -> pd.DataFrame:
    ...
    if not csv_path.exists() or csv_path.stat().st_size == 0:
        return pd.DataFrame()
    try:
        return pd.read_csv(csv_path, dtype=str, keep_default_na=False, encoding='utf-8-sig').fillna('')
...
def status_name_for_role(workflow_role: str, status_file: str | Path=MAP_STATUS_FILE) -> str:
    ...
    if not definitions.empty:
        for column in ['Status Name', 'Active', 'Workflow Role']:
            if column not in definitions.columns:
                definitions[column] = ''
        matches = definitions[definitions['Workflow Role'].astype(str).str.casefold().eq(role.casefold()) & definitions['Active'].astype(str).str.casefold().eq('yes')]
...
def set_unit_workflow_role(serial_number: Any, workflow_role: str, *, clear_manual_colour: bool=True, state_file: str | Path=MAP_UNIT_STATE_FILE, status_file: str | Path=MAP_STATUS_FILE) -> None:
    ...
    if mask.any():
        ...
        state = pd.concat([state, pd.DataFrame([{'Serial Number': serial, 'Status': status_name, 'Color Override': '', 'Updated At': _now_text()}], columns=STATE_COLUMNS)], ignore_index=True)
    _save_state(state, state_file)

def unresolved_issue_rows(issue_csv_file: str | Path, serial_number: Any) -> pd.DataFrame:
    serial = clean_text(serial_number)
    ...
    for column in ['Serial Number', 'Status']:
        if column not in issues.columns:
            issues[column] = ''
    mask = issues['Serial Number'].astype(str).str.casefold().eq(serial.casefold()) & ~issues['Status'].astype(str).str.casefold().isin({'closed', 'resolved'})
    return issues.loc[mask].copy()

def sync_unit_from_issue_records(issue_csv_file: str | Path, serial_number: Any, *, clear_role: str='Completed', state_file: str | Path | None=None, status_file: str | Path | None=None) -> str:
    ...
    if unresolved.empty:
        ...
        role = 'Pending Verification' if statuses.eq('pending verification').all() else 'Issue'
    ...
    set_unit_workflow_role(serial, role, clear_manual_colour=True, state_file=effective_state_file, status_file=effective_status_file)
    return role
