...
import requests
from config import EXCEL_FILE, MICROSOFT_CONFIG, ONEDRIVE_CLOUD_ENABLED, ONEDRIVE_PENDING_DIR, ONEDRIVE_SYNC_STATE_FILE
...
def _token() -> str:
    ...
    if not token:
        raise OneDriveAuthenticationError('Microsoft sign-in has expired. Sign in again before using OneDrive.')
    return token
...
def _request_error(response: requests.Response, action: str) -> OneDriveError:
    ...
    if response.status_code == 404:
        return OneDriveError(f'The OneDrive workbook was not found at {_remote_path()}.')
    if response.status_code in {409, 412, 423}:
        return OneDriveConflictError('The OneDrive workbook changed or is temporarily locked. Refresh AED Data and try again.')
    try:
        detail = response.json().get('error', {}).get('message', '')
    except (ValueError, AttributeError):
        detail = response.text[:500]
    return OneDriveError(f'OneDrive {action} failed ({response.status_code}). {detail}'.strip())
...
def get_remote_metadata() -> dict[str, Any]:
    if not ONEDRIVE_CLOUD_ENABLED:
        return {}
    response = requests.get(_item_url(), headers=_headers(_token()), params={'$select': 'id,name,size,eTag,cTag,lastModifiedDateTime,webUrl'}, timeout=REQUEST_TIMEOUT_SECONDS)
    if not response.ok:
...
def download_workbook(*, force: bool=False) -> OneDriveSyncResult:
    ...
    if not ONEDRIVE_CLOUD_ENABLED:
        return OneDriveSyncResult('local_mode', 'Using the project-local Excel workbook.', changed=False, source_exists=Path(EXCEL_FILE).exists())
    state = load_onedrive_state()
    local = Path(EXCEL_FILE)
    if not force and local.exists() and (str(state.get('remote_path', '')) == _remote_path()) and _recent_metadata_check(state):
        return OneDriveSyncResult('up_to_date', 'OneDrive Excel was checked recently.', changed=False, source_exists=True, etag=str(state.get('etag', '') or ''), last_modified=str(state.get('last_modified', '') or ''), remote_path=_remote_path(), item_id=str(state.get('item_id', '') or ''))
    metadata = get_remote_metadata()
    etag = str(metadata.get('eTag', '') or '')
    if not force and local.exists() and etag and (etag == str(state.get('etag', '') or '')) and (str(state.get('remote_path', '')) == _remote_path()):
        checked = datetime.now().astimezone().isoformat(timespec='seconds')
        refreshed_state = dict(state)
        refreshed_state.update({'last_metadata_check_time': checked, 'last_modified': str(metadata.get('lastModifiedDateTime', '') or ''), 'item_id': str(metadata.get('id', '') or '')})
        _save_state(refreshed_state)
        return OneDriveSyncResult('up_to_date', 'OneDrive Excel is already up to date.', changed=False, source_exists=True, etag=etag, last_modified=str(metadata.get('lastModifiedDateTime', '') or ''), remote_path=_remote_path(), item_id=str(metadata.get('id', '') or ''))
    response = requests.get(_content_url(), headers=_headers(_token()), timeout=REQUEST_TIMEOUT_SECONDS, allow_redirects=True)
    if not response.ok:
        raise _request_error(response, 'download')
    _atomic_write_local(response.content, local)
    now = datetime.now().astimezone().isoformat(timespec='seconds')
    _save_state({'remote_path': _remote_path(), 'item_id': str(metadata.get('id', '') or ''), 'etag': etag, 'last_modified': str(metadata.get('lastModifiedDateTime', '') or ''), 'last_download_time': now, 'last_metadata_check_time': now, 'last_upload_time': str(state.get('last_upload_time', '') or ''), 'web_url': str(metadata.get('webUrl', '') or '')})
    return OneDriveSyncResult('downloaded', 'Latest Excel downloaded from OneDrive.', changed=True, source_exists=True, etag=etag, last_modified=str(metadata.get('lastModifiedDateTime', '') or ''), remote_path=_remote_path(), item_id=str(metadata.get('id', '') or ''))
...
def upload_workbook(*, expected_etag: str='') -> OneDriveSyncResult:
    ...
    if not ONEDRIVE_CLOUD_ENABLED:
        return OneDriveSyncResult('local_mode', 'Local Excel saved.', changed=True, source_exists=Path(EXCEL_FILE).exists())
    local = Path(EXCEL_FILE)
    if not local.exists():
    ...
    if expected and current_etag and (expected != current_etag):
        raise OneDriveConflictError('The OneDrive Excel file changed after this page was loaded. Refresh AED Data before saving.')
    item_id = str(current.get('id', '') or '')
    if not item_id:
        raise OneDriveError('OneDrive did not return the workbook item ID.')
    headers = _headers(_token(), **{'Content-Type': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'})
    response = requests.put(f"{GRAPH_ROOT}/me/drive/items/{quote(item_id, safe='')}/content", headers=headers, data=local.read_bytes(), timeout=REQUEST_TIMEOUT_SECONDS)
    if not response.ok:
    ...
    now = datetime.now().astimezone().isoformat(timespec='seconds')
    state = load_onedrive_state()
    _save_state({'remote_path': _remote_path(), 'item_id': str(payload.get('id', current.get('id', '')) or ''), 'etag': new_etag, 'last_modified': str(payload.get('lastModifiedDateTime', '') or ''), 'last_download_time': str(state.get('last_download_time', '') or ''), 'last_metadata_check_time': now, 'last_upload_time': now, 'web_url': str(payload.get('webUrl', current.get('webUrl', '')) or '')})
    return OneDriveSyncResult('uploaded', 'OneDrive Excel updated successfully.', changed=True, source_exists=True, etag=new_etag, last_modified=str(payload.get('lastModifiedDateTime', '') or ''), remote_path=_remote_path(), item_id=str(payload.get('id', '') or ''))
