...
_TOKEN_CACHE: dict[str, Any] = {'access_token': '', 'expiry_timestamp': 0}
...
def _credentials_available() -> bool:
    return bool(_read_setting('ONEMAP_EMAIL') and _read_setting('ONEMAP_PASSWORD'))

def _request_new_access_token() -> str:
    email = _read_setting('ONEMAP_EMAIL')
    password = _read_setting('ONEMAP_PASSWORD')
    if not email or not password:
        raise RuntimeError('OneMap credentials are missing. Add ONEMAP_EMAIL and ONEMAP_PASSWORD to .streamlit/secrets.toml.')
    response = requests.post(ONEMAP_AUTH_URL, json={'email': email, 'password': password}, timeout=20)
    response.raise_for_status()
    data = response.json()
    access_token = str(data.get('access_token', '')).strip()
    if not access_token:
        raise RuntimeError(str(data.get('error', 'OneMap did not return an access token.')))
    try:
...
def get_access_token(force_refresh: bool=False) -> str:
    ...
    if not force_refresh and cached_token and (cached_expiry > now + 60):
        return cached_token
...
def _search_postal_code(postal_code: str, access_token: str) -> dict[str, Any]:
    response = requests.get(ONEMAP_SEARCH_URL, params={'searchVal': postal_code, 'returnGeom': 'Y', 'getAddrDetails': 'Y', 'pageNum': 1}, headers={'Authorization': access_token}, timeout=20)
    response.raise_for_status()
...
def geocode_postal_code(value: Any) -> GeocodingResult:
    ...
    if not postal_code:
        return GeocodingResult(status='Invalid Postal Code')
    try:
        ...
        if token_error and _credentials_available():
            ...
            token_error = str(data.get('error', '')).strip()
        if token_error:
            return GeocodingResult(status=f'OneMap Error: {token_error}')
        ...
        if exact_match is None:
            return GeocodingResult(status='Postal Code Not Found')
        latitude = str(exact_match.get('LATITUDE', '')).strip()
        longitude = str(exact_match.get('LONGITUDE', exact_match.get('LONGTITUDE', ''))).strip()
        address = str(exact_match.get('ADDRESS', '')).strip()
        if not latitude or not longitude:
            return GeocodingResult(address=address, status='Coordinates Missing in OneMap Response')
        float(latitude)
        float(longitude)
        return GeocodingResult(latitude=latitude, longitude=longitude, address=address, status='Success')
    except requests.HTTPError as error:
        status_code = error.response.status_code if error.response else ''
        return GeocodingResult(status=f'OneMap HTTP Error {status_code}'.strip())
    except requests.RequestException as error:
        return GeocodingResult(status=f'OneMap Network Error: {error}')
    except (RuntimeError, TypeError, ValueError) as error:
        return GeocodingResult(status=f'OneMap Configuration Error: {error}')
