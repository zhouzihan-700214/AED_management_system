...
def _required_config() -> dict[str, str]:
    if not ONEDRIVE_CLOUD_ENABLED:
        raise RuntimeError('Microsoft OneDrive configuration is incomplete. Check Streamlit Settings > Secrets.')
...
def _signed_state() -> str:
    ...
    signature = hmac.new(config['client_secret'].encode('utf-8'), payload.encode('utf-8'), hashlib.sha256).hexdigest()
...
def _valid_state(value: str) -> bool:
    ...
    expected = hmac.new(config['client_secret'].encode('utf-8'), payload.encode('utf-8'), hashlib.sha256).hexdigest()
    return hmac.compare_digest(expected, supplied_signature)

def build_sign_in_url() -> str:
    config = _required_config()
    params = {'client_id': config['client_id'], 'response_type': 'code', 'redirect_uri': config['redirect_uri'], 'response_mode': 'query', 'scope': ' '.join(GRAPH_SCOPES), 'state': _signed_state(), 'prompt': 'select_account'}
...
def _token_error(response: requests.Response) -> str:
    try:
        ...
        return str(payload.get('error_description') or payload.get('error') or response.text[:500])
    except ValueError:
...
def _load_account_name(access_token: str) -> str:
    try:
        response = requests.get('https://graph.microsoft.com/v1.0/me', headers={'Authorization': f'Bearer {access_token}'}, params={'$select': 'displayName,userPrincipalName,mail'}, timeout=REQUEST_TIMEOUT_SECONDS)
        if response.ok:
            payload = response.json()
            return str(payload.get('mail') or payload.get('userPrincipalName') or payload.get('displayName') or 'Microsoft account')
...
def handle_auth_callback() -> bool:
    ...
    if params.get('error'):
        st.session_state[AUTH_ERROR_KEY] = params.get('error_description') or params['error']
    ...
    if not _valid_state(params.get('state', '')):
        st.session_state[AUTH_ERROR_KEY] = 'The Microsoft sign-in response could not be verified. Please sign in again.'
        st.query_params.clear()
        return False
    config = _required_config()
    response = requests.post(_authority_endpoint('token'), data={'client_id': config['client_id'], 'client_secret': config['client_secret'], 'grant_type': 'authorization_code', 'code': code, 'redirect_uri': config['redirect_uri'], 'scope': ' '.join(GRAPH_SCOPES)}, timeout=REQUEST_TIMEOUT_SECONDS)
    st.query_params.clear()
    if not response.ok:
...
def _refresh_access_token(refresh_token: str) -> str | None:
    st = _streamlit()
    config = _required_config()
    response = requests.post(_authority_endpoint('token'), data={'client_id': config['client_id'], 'client_secret': config['client_secret'], 'grant_type': 'refresh_token', 'refresh_token': refresh_token, 'redirect_uri': config['redirect_uri'], 'scope': ' '.join(GRAPH_SCOPES)}, timeout=REQUEST_TIMEOUT_SECONDS)
    if not response.ok:
        st.session_state[AUTH_ERROR_KEY] = _token_error(response)
        return None
    payload = response.json()
...
def get_authentication_status() -> AuthenticationStatus:
    ...
    return AuthenticationStatus(False, '', str(st.session_state.get(AUTH_ERROR_KEY, '')))
