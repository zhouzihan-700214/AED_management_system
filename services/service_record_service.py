...
def load_aed_lookup(aed_csv_file: str | Path | None=None, *, aed_dataframe: pd.DataFrame | None=None) -> pd.DataFrame:
    ...
    """
    del aed_csv_file
    dataframe = aed_dataframe.copy() if aed_dataframe is not None else get_all_units().copy()
...
def add_record_match_status(records: pd.DataFrame, aed_lookup: pd.DataFrame) -> pd.DataFrame:
...
def service_record_scope_counts(records: pd.DataFrame) -> dict[str, int]:
    ...
    return {'All Records': int(len(records)), 'Matched': int(status.eq('Matched').sum()), 'Mismatch': int(status.eq('Mismatch').sum()), 'Loaner': int(status.eq('Loaner').sum())}

def manual_records_for_service_page(manual_service_file: str | Path) -> pd.DataFrame:
    ...
    legacy_battery = (manual['Service Type'].astype(str).str.contains('batt|battery', case=False, regex=True, na=False) & manual['Status'].astype(str).str.strip().str.casefold().eq('completed')).map({True: 'Yes', False: 'No'})
    result['Battery Replaced'] = battery_replaced.where(battery_replaced.ne(''), legacy_battery)
...
def issue_resolution_records_for_service_page(issue_record_file: str | Path=ISSUE_RECORD_FILE, resolution_file: str | Path=ISSUE_RESOLUTION_FILE) -> pd.DataFrame:
    ...
    if issues.empty or resolutions.empty or 'Issue ID' not in resolutions.columns:
        return pd.DataFrame()
    issue_columns = ['Issue ID', 'Serial Number', 'Model', 'Location', 'Postal Code', 'Lift Lobby', 'Is Loaner', 'Issue Type', 'Status']
    for column in issue_columns:
        if column not in issues.columns:
            issues[column] = ''
    merged = resolutions.merge(issues[issue_columns], on='Issue ID', how='left', suffixes=('', '_Issue')).fillna('')
    if merged.empty:
        return pd.DataFrame()
    ...
    if not attachments.empty and 'Submission ID' in attachments.columns:
        for submission_id, group in attachments.groupby(attachments['Submission ID'].astype(str).str.strip(), dropna=False):
            key = clean_text(submission_id)
            if not key:
                continue
            attachment_map[key] = [clean_text(value) for value in group.get('File Path', pd.Series(dtype=str)).tolist() if clean_text(value)]
    ...
    result['Attachment Paths'] = submission_ids.map(lambda value: '; '.join(attachment_map.get(clean_text(value), [])))
    result['Attachment Count'] = submission_ids.map(lambda value: str(len(attachment_map.get(clean_text(value), []))))
    detail_columns = [('Issue', 'Issue Type'), ('Action', 'Action Taken'), ('Root cause', 'Root Cause'), ('Parts', 'Parts Replaced'), ('Test', 'Test Performed'), ('Test result', 'Test Result'), ('Resolution', 'Resolution Notes'), ('Verification', 'Verification Notes')]
    details: list[str] = []
    ...
    combined_text = merged.get('Action Taken', pd.Series('', index=merged.index)).astype(str) + ' ' + merged.get('Parts Replaced', pd.Series('', index=merged.index)).astype(str)
    result['Battery Replaced'] = combined_text.str.contains('battery|batt', case=False, regex=True, na=False).map({True: 'Yes', False: 'No'})
    return result.fillna('')

def load_service_records(response_csv_file: str | Path, aed_csv_file: str | Path | None=None, manual_service_file: str | Path=MANUAL_SERVICE_RECORDS_FILE, issue_record_file: str | Path=ISSUE_RECORD_FILE, resolution_file: str | Path=ISSUE_RESOLUTION_FILE, *, aed_dataframe: pd.DataFrame | None=None) -> pd.DataFrame:
    ...
    if response_path.exists() and response_path.stat().st_size > 0:
        records = pd.read_csv(response_path, dtype=str, keep_default_na=False, encoding='utf-8-sig').fillna('')
        records['Record Source'] = records.get('Record Source', 'PM Checklist')
        if isinstance(records['Record Source'], pd.Series):
            records['Record Source'] = records['Record Source'].replace('', 'PM Checklist')
        if 'Record Status' not in records.columns:
            ...
            records.loc[blank_status, 'Record Status'] = records.loc[blank_status, 'Submission Status'] if 'Submission Status' in records.columns else ''
    ...
    resolutions = issue_resolution_records_for_service_page(issue_record_file=issue_record_file, resolution_file=resolution_file)
    ...
    required_columns = {'Service Date', 'Technician', 'Service Type', 'Postal Code', 'Lift Lobby', 'Loaner Unit', 'Record Match', 'AED Serial Number', 'Battery Replaced', 'Adult Pads Lot Number', 'Pediatric Pads Lot Number', 'AED Location', 'Submitted At', 'PM Response ID', 'Original Serial Number', 'Record Source', 'Record Status', 'Submission Status', 'Excel Update Status', 'Submitted By', 'Operation ID', 'Service Notes', 'Service Report e-SR', 'PM Dates Updated', 'Battery History Updated', 'PM Interval Months Used', 'Master Operation ID', 'Linked Plan ID', 'Failed Checklist Fields', 'Created Issue IDs', 'Issue ID', 'Issue Type', 'Resolution Submission ID', 'Resolution Attempt Number', 'Action Taken', 'Root Cause', 'Parts Replaced', 'Test Performed', 'Test Result', 'Resolution Notes', 'Verification Result', 'Verification Notes', 'Verified By', 'Verified At', 'Attachment Count', 'Attachment Paths'}
    for column in required_columns:
        if column not in records.columns:
            records[column] = ''
    if 'AED Model' not in records.columns:
        records['AED Model'] = ''
    aed_lookup = load_aed_lookup(aed_csv_file, aed_dataframe=aed_dataframe)
    ...
    for row_index, row in records.iterrows():
        ...
        serial_keys = [serial.casefold() for serial in [current_serial, original_serial] if serial]
