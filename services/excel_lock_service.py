...
def inspect_lock(lock_file: str | Path=EXCEL_OPERATION_LOCK_FILE) -> dict[str, Any]:
    ...
    return {'exists': True, 'active': not confirmed_stale, 'confirmed_stale': confirmed_stale, 'stale': confirmed_stale, 'warning': warning, 'stale_by_age': stale_by_age, 'age_minutes': age_minutes, 'same_host': same_host, 'process_alive': process_alive, 'payload': payload}
...
class ExcelOperationLock:
    ...
    def acquire(self, metadata: dict[str, Any]) -> None:
        ...
        payload = {**metadata, 'hostname': socket.gethostname(), 'process_id': os.getpid(), 'started_at': metadata.get('started_at') or now_iso()}
        try:
            descriptor = os.open(self.path, os.O_CREAT | os.O_EXCL | os.O_WRONLY, 384)
        except FileExistsError as error:
            current = inspect_lock(self.path)
            existing = current.get('payload', {})
            details = f" Operation: {existing.get('operation_type', 'unknown')}; user: {existing.get('user', 'unknown')}; started: {existing.get('started_at', 'unknown')}."
            raise RuntimeError('Another Excel operation is in progress. Try again after it finishes.' + details) from error
