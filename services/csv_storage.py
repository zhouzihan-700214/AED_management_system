...
def atomic_write_csv(dataframe: pd.DataFrame, csv_file: str | Path, *, preferred_columns: Iterable[str] | None=None) -> None:
    ...
    try:
        with tempfile.NamedTemporaryFile(mode='w', suffix='.csv', prefix=f'.{target_path.stem}_', dir=target_path.parent, delete=False, encoding='utf-8-sig', newline='') as temporary_file:
...
def read_csv_safe(csv_file: str | Path, *, columns: Iterable[str] | None=None) -> pd.DataFrame:
