...
from config import ACTIVE_TRANSACTION_FILE, AED_CACHE_FILE, AED_LIFECYCLE_FILE, CACHE_BACKUP_DIR, EXCEL_BACKUP_DIR, EXCEL_DATA_START_ROW, EXCEL_FILE, EXCEL_HEADER_ROW, EXCEL_OPERATION_LOCK_FILE, EXCEL_SHEET, MAX_CACHE_BACKUPS, MAX_EXCEL_BACKUPS, PRESERVE_CACHE_ONLY_UNITS, STAGING_SHEET_NAME, SYNC_STATE_FILE, TEMP_DIR
...
from services.excel_write_service import APP_TO_EXCEL_COLUMNS, DATE_FIELDS, _capture_baseline, _copy_stable_workbook, _continuation_rows_after, _create_excel_backup, _display_value, _find_serial_row, _header_map, _make_temporary_workbook_path, _normalise_changes, _normalise_lift_lobby, _read_app_value, _read_combined_remarks, _required_column, _serial_rows, _validate_structure, _validate_target_values, _write_cell, _write_combined_remarks
from utils.text_utils import clean_text
ADD_TO_EXCEL_COLUMNS = dict(APP_TO_EXCEL_COLUMNS)
DATE_ADD_FIELDS = set(DATE_FIELDS)
LIFECYCLE_COLUMNS = ['Timestamp', 'Operation ID', 'User', 'Session ID', 'Serial Number', 'Status', 'Reason', 'Source Page']
...
def _finish_journal(operation: dict[str, Any], result: str, message: str, *, clear_active: bool=True) -> None:
    record_transaction(**{'Operation ID': operation['operation_id'], 'Operation Type': operation['operation_type'], 'User': operation.get('user', ''), 'Session ID': operation.get('session_id', ''), 'Serial Numbers': '; '.join(operation.get('serial_numbers', [])), 'Source Page': operation.get('source_page', ''), 'Started At': operation.get('started_at', ''), 'Result': result, 'Final Step': operation.get('last_completed_step', ''), 'Message': message})
    if clear_active:
...
def _new_operation(operation_type: str, user: str, session_id: str, serials: Sequence[str], source_page: str) -> dict[str, Any]:
    ...
    return {'operation_id': operation_id, 'operation_type': operation_type, 'status': 'STARTED', 'user': clean_text(user), 'session_id': clean_text(session_id), 'serial_numbers': [clean_text(item) for item in serials], 'source_page': clean_text(source_page), 'source_file': str(EXCEL_FILE), 'temp_file': None, 'backup_file': None, 'started_at': _now_iso(), 'last_completed_step': 'STARTED'}

def _sync_without_relocking() -> SyncResult:
    return sync_excel_to_cache(force=True, excel_file=EXCEL_FILE, excel_sheet=EXCEL_SHEET, cache_file=AED_CACHE_FILE, state_file=SYNC_STATE_FILE, temp_dir=TEMP_DIR, lock_file=EXCEL_OPERATION_LOCK_FILE, backup_dir=CACHE_BACKUP_DIR, preserve_cache_only_units=PRESERVE_CACHE_ONLY_UNITS, max_backups=MAX_CACHE_BACKUPS, acquire_lock=False)

def _audit_rows(operation: Mapping[str, Any], serial: str, desired: Mapping[str, Any], original: Mapping[str, Any], current: Mapping[str, Any], result: str, message: str) -> list[dict[str, Any]]:
    return [{'Operation ID': operation['operation_id'], 'User': operation.get('user', ''), 'Session ID': operation.get('session_id', ''), 'Operation Type': operation.get('operation_type', ''), 'Source Page': operation.get('source_page', ''), 'Serial Number': serial, 'Field': field_name, 'Original Value': original.get(field_name, ''), 'Current Value': current.get(field_name, ''), 'Desired Value': _display_value(field_name, desired_value), 'Result': result, 'Message': message} for field_name, desired_value in desired.items()]

def execute_unit_update(*, serial_number: str, desired_values: Mapping[str, Any], original_values: Mapping[str, Any], user: str, session_id: str, source_page: str) -> OperationResult:
    ...
    try:
        with operation_lock({'operation_id': operation['operation_id'], 'operation_type': 'UPDATE_UNIT', 'user': changed_by, 'session_id': session, 'serial_numbers': [serial]}, EXCEL_OPERATION_LOCK_FILE):
            ...
            for field_name, desired_value in normalised.items():
                ...
                if field_name == 'Remarks':
                    current[field_name] = _read_combined_remarks(worksheet, row_number=row_number, serial_column=serial_col, remarks_column=col)
                else:
                    current[field_name] = _read_app_value(worksheet.cell(row=row_number, column=col), field_name)
            ...
            if conflict_check['conflicts']:
                ...
                return OperationResult('conflict', message, operation_id=operation['operation_id'], serial_number=serial, conflicts=conflict_check['conflicts'], already_applied=tuple(conflict_check['already_applied']))
            ...
            if not safe_changes:
                ...
                return OperationResult('already_applied', message, operation_id=operation['operation_id'], serial_number=serial, already_applied=tuple(conflict_check['already_applied']))
            ...
            for field_name, value in safe_changes.items():
                staging.append([operation['operation_id'], serial, field_name, row_number, original_display.get(field_name, ''), current.get(field_name, ''), _display_value(field_name, value)])
                if field_name == 'Remarks':
                    allowed_continuation_rows.update(_write_combined_remarks(worksheet, row_number=row_number, serial_column=serial_col, remarks_column=columns[field_name], value=value))
                else:
                    _write_cell(worksheet.cell(row=row_number, column=columns[field_name]), field_name, value)
            ...
            return OperationResult('updated', message, operation_id=operation['operation_id'], serial_number=serial, serial_numbers=(serial,), changed_fields=tuple(safe_changes), backup_file=str(backup or ''), cache_sync_status=sync_result.status, warnings=tuple(sync_result.warnings), already_applied=tuple(conflict_check['already_applied']))
        ...
        try:
            ...
            pass
        return OperationResult('partial' if source_replaced else 'failed', 'Excel was updated, but website refresh failed. Recovery can safely refresh the cache. ' + message if source_replaced else message, operation_id=operation['operation_id'], serial_number=serial, backup_file=str(backup or ''))
    finally:
        if temp_path is not None:
            temp_path.unlink(missing_ok=True)

def execute_batch_updates(*, updates: Sequence[Mapping[str, Any]], user: str, session_id: str, source_page: str) -> OperationResult:
    ...
    source_replaced = False
    try:
        with operation_lock({'operation_id': operation['operation_id'], 'operation_type': 'BATCH_UPDATE', 'user': user, 'session_id': session_id, 'serial_numbers': serials}, EXCEL_OPERATION_LOCK_FILE):
            _journal(operation, 'LOCK_ACQUIRED', 'LOCK_ACQUIRED')
            temp_path = _make_temporary_workbook_path(Path(EXCEL_FILE))
            operation['temp_file'] = str(temp_path)
            _copy_stable_workbook(Path(EXCEL_FILE), temp_path)
            ...
            for item in cleaned:
                ...
                for field, value in desired.items():
                    ...
                    if field == 'Remarks':
                        current[field] = _read_combined_remarks(ws, row_number=row, serial_column=serial_col, remarks_column=col)
                    else:
                        current[field] = _read_app_value(ws.cell(row=row, column=col), field)
            ...
            if all_conflicts:
                ...
                for serial, fields in all_conflicts.items():
                    for field, values in fields.items():
                        conflict_rows.append({'Operation ID': operation['operation_id'], 'User': user, 'Session ID': session_id, 'Operation Type': 'BATCH_UPDATE', 'Source Page': source_page, 'Serial Number': serial, 'Field': field, 'Original Value': values['original'], 'Current Value': values['current'], 'Desired Value': values['desired'], 'Result': 'Conflict', 'Message': message})
            ...
            staging = wb.create_sheet(STAGING_SHEET_NAME)
            staging.sheet_state = 'hidden'
            staging.append(['Operation ID', 'Serial Number', 'Field', 'Excel Row', 'Original', 'Current', 'Desired'])
            ...
            for serial, row, safe, original, current, desired_display in staged:
                ...
                for field, value in safe.items():
                    ...
                    col = _required_column(headers, APP_TO_EXCEL_COLUMNS[field])
                    if field == 'Remarks':
                        allowed_continuation_rows.update(_write_combined_remarks(ws, row_number=row, serial_column=serial_col, remarks_column=col, value=value))
                    else:
...
def execute_add_unit(*, values: Mapping[str, Any], user: str, session_id: str, source_page: str) -> OperationResult:
    ...
    try:
        with operation_lock({'operation_id': operation['operation_id'], 'operation_type': 'ADD_UNIT', 'user': user, 'session_id': session_id, 'serial_numbers': [serial]}, EXCEL_OPERATION_LOCK_FILE):
            ...
            wb.save(temp_path)
            wb.close()
            check = load_workbook(temp_path, data_only=False)
            cws = check[EXCEL_SHEET]
            ...
            del check[STAGING_SHEET_NAME]
            check.save(temp_path)
            check.close()
            ...
            record_field_audit([{'Operation ID': operation['operation_id'], 'User': user, 'Session ID': session_id, 'Operation Type': 'ADD_UNIT', 'Source Page': source_page, 'Serial Number': serial, 'Field': 'Record', 'Original Value': '', 'Current Value': '', 'Desired Value': 'New AED', 'Result': 'Success', 'Message': message}])
            _journal(operation, 'COMMITTED', 'COMMITTED')
            _finish_journal(operation, 'Success', message)
...
def execute_deactivate_unit(*, serial_number: str, user: str, session_id: str, source_page: str, reason: str='') -> OperationResult:
    ...
    try:
        with operation_lock({'operation_id': operation['operation_id'], 'operation_type': 'DEACTIVATE_UNIT', 'user': user, 'session_id': session_id, 'serial_numbers': [serial]}, EXCEL_OPERATION_LOCK_FILE):
            ...
            headers = _header_map(ws, EXCEL_HEADER_ROW)
            serial_col = _required_column(headers, 'SERIAL NUMBER')
            _find_serial_row(ws, serial_column=serial_col, data_start_row=EXCEL_DATA_START_ROW, serial_number=serial)
            wb.close()
            _append_lifecycle({'Timestamp': _now_iso(), 'Operation ID': operation['operation_id'], 'User': user, 'Session ID': session_id, 'Serial Number': serial, 'Status': 'Inactive', 'Reason': clean_text(reason), 'Source Page': source_page})
            message = f'{serial} was marked inactive. Its Excel row and service history were preserved.'
            record_field_audit([{'Operation ID': operation['operation_id'], 'User': user, 'Session ID': session_id, 'Operation Type': 'DEACTIVATE_UNIT', 'Source Page': source_page, 'Serial Number': serial, 'Field': 'Lifecycle Status', 'Original Value': 'Active', 'Current Value': 'Active', 'Desired Value': 'Inactive', 'Result': 'Success', 'Message': message}])
            _journal(operation, 'COMMITTED', 'COMMITTED')
            _finish_journal(operation, 'Success', message)
        ...
        message = str(error)
        try:
            _journal(operation, 'FAILED', 'FAILED', error=message)
            _finish_journal(operation, 'Failed', message)
        except OSError:
            pass
