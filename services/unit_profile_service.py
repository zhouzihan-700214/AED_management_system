...
SERVICE_HISTORY_COLUMNS = ['Service Date', 'Service Type', 'Source', 'Technician', 'Reference', 'Status', 'Details']
MANUAL_SERVICE_PREFIX = '[SERVICE]'
MANUAL_SERVICE_RECORD_COLUMNS = ['Service Record ID', 'Created At', 'Created By', 'AED Serial Number', 'AED Model', 'AED Location', 'Postal Code', 'Lift Lobby', 'Service Date', 'Service Type', 'Technician', 'Reference', 'Status', 'Details', 'Master Data Updated', 'PM Dates Updated', 'Battery Replaced', 'Battery History Updated', 'PM Interval Months Used', 'Linked Plan ID', 'Master Operation ID', 'Source']
...
def build_manual_service_update_plan(master_row: Mapping[str, Any], *, service_date: date, service_type: str, technician: str, reference: str, status: str, update_latest: bool, update_pm_dates: bool, interval_months: int) -> dict[str, Any]:
    ...
    if (update_latest or update_pm_dates) and (not completed):
        raise ValueError('Pending or follow-up records cannot update the latest completed service fields.')
    ...
    if update_pm_dates:
        ...
        add_change('Next PM Date', calculate_next_pm_date(service_date_text, interval))
    type_key = service_type_text.casefold()
    battery_replaced = completed and ('batt' in type_key or 'battery' in type_key)
    if battery_replaced:
        old_history = normalize_value('Battery Replacement History', master_row.get('Battery Replacement History', ''))
    ...
    return {'changes': changes, 'originals': originals, 'service_date_text': service_date_text, 'battery_replaced': battery_replaced, 'master_fields_changed': sorted(set(changes) & latest_fields), 'pm_fields_changed': sorted(set(changes) & pm_fields), 'battery_history_changed': 'Battery Replacement History' in changes, 'interval_months_used': str(interval) if update_pm_dates else '', 'complete_pm_plan': bool(completed and update_pm_dates and service_type_text.casefold().startswith('pm'))}

def ensure_manual_service_storage(path: str | Path=MANUAL_SERVICE_RECORDS_FILE) -> Path:
...
def load_manual_service_records(path: str | Path=MANUAL_SERVICE_RECORDS_FILE) -> pd.DataFrame:
...
def append_manual_service_record(record: Mapping[str, Any], *, path: str | Path=MANUAL_SERVICE_RECORDS_FILE) -> dict[str, str]:
    record_path = ensure_manual_service_storage(path)
    ...
    duplicate = existing[existing['Service Record ID'].astype(str).str.strip().eq(saved['Service Record ID'])]
    if not duplicate.empty:
        return {column: clean_text(duplicate.iloc[0].get(column, '')) for column in MANUAL_SERVICE_RECORD_COLUMNS}
    updated = pd.concat([existing, pd.DataFrame([saved], columns=MANUAL_SERVICE_RECORD_COLUMNS)], ignore_index=True)
    atomic_write_csv(updated, record_path, preferred_columns=MANUAL_SERVICE_RECORD_COLUMNS)
    return saved
...
def _manual_service_rows(serial: str, path: str | Path) -> list[dict[str, str]]:
    ...
    for _, row in matches.iterrows():
        ...
        rows.append({'Service Date': clean_text(row.get('Service Date')), 'Service Type': clean_text(row.get('Service Type')) or 'Service', 'Source': clean_text(row.get('Source')) or 'Unit Profile', 'Technician': clean_text(row.get('Technician')), 'Reference': clean_text(row.get('Reference')) or clean_text(row.get('Service Record ID')), 'Status': clean_text(row.get('Status')) or 'Recorded', 'Details': ' · '.join((part for part in detail_parts if part))})
    return rows

def format_manual_service_remark(*, service_date: str, service_type: str, technician: str='', reference: str='', status: str='Completed', details: str='') -> str:
    """Create one stable, human-readable service-history line for Remarks."""
    parts = [f'{MANUAL_SERVICE_PREFIX} {clean_text(service_date)}', f"Type: {clean_text(service_type) or 'Service'}"]
    ...
    if clean_text(details):
        compact = re.sub('\\s+', ' ', clean_text(details))
...
def _parse_structured_service_remark(piece: str) -> dict[str, str] | None:
    ...
    return {'Service Date': _extract_date(first) or first, 'Service Type': fields.get('type', 'Service'), 'Source': 'Profile Service Record', 'Technician': fields.get('technician', ''), 'Reference': fields.get('reference', ''), 'Status': fields.get('status', 'Recorded'), 'Details': fields.get('details', '')}
_DATE_PATTERNS = [re.compile('\\b\\d{1,2}[-/]\\d{1,2}[-/]\\d{2,4}\\b'), re.compile('\\b\\d{1,2}\\s+(?:Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:tember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)\\s+\\d{4}\\b', flags=re.IGNORECASE)]

def _matching_rows(dataframe: pd.DataFrame, serial: str, columns: tuple[str, ...]) -> pd.DataFrame:
    if dataframe.empty:
...
def _extract_reference(text: str) -> str:
    value = clean_text(text)
    match = re.search('\\be[- ]?SR\\s*[-:]?\\s*\\d+\\b', value, flags=re.IGNORECASE)
    if not match:
        return ''
    return re.sub('\\s+', '', match.group(0)).replace('ESR', 'e-SR')
...
def _legacy_remark_rows(master_row: pd.Series) -> list[dict[str, str]]:
    ...
    if not remarks:
        return []
    pieces = [clean_text(piece).strip(' ,-/') for piece in re.split('\\s*/\\s*|[\\r\\n]+|\\s*;\\s*|(?=\\bPM\\s+(?:completed|done)\\b)|(?=\\bCommissioning\\b)|(?=\\bRepair(?:ed)?\\b)|(?=\\bIncoming Check\\b)|(?=\\bOutgoing Check\\b)', remarks, flags=re.IGNORECASE) if clean_text(piece).strip(' ,-/')]
    if not pieces:
        pieces = [remarks]
    rows: list[dict[str, str]] = []
    for piece in pieces:
        ...
        if structured is not None:
            ...
            continue
        rows.append({'Service Date': _extract_date(piece), 'Service Type': _infer_service_type(piece), 'Source': 'IB List Remarks', 'Technician': '', 'Reference': _extract_reference(piece), 'Status': 'Historical', 'Details': piece})
    return rows

def _pm_response_rows(pm_responses: pd.DataFrame, serial: str) -> list[dict[str, str]]:
    matches = _matching_rows(pm_responses, serial, ('AED Serial Number', 'Original Serial Number'))
    rows: list[dict[str, str]] = []
    for _, row in matches.iterrows():
        detail_parts = []
        for label, column in [('Battery replaced', 'Battery Replaced'), ('Cabinet', 'Cabinet Inspection'), ('Alarm', 'Cabinet Alarm'), ('Self test', 'Self Test Result'), ('Final check', 'Final Check'), ('Notes', 'Service Notes'), ('Linked plan', 'Linked Plan ID'), ('Created Issues', 'Created Issue IDs')]:
            value = clean_text(row.get(column))
            if value:
                detail_parts.append(f'{label}: {value}')
        rows.append({'Service Date': clean_text(row.get('Service Date')) or clean_text(row.get('Submitted At')), 'Service Type': clean_text(row.get('Service Type')) or 'PM', 'Source': 'PM Checklist', 'Technician': clean_text(row.get('Technician')) or clean_text(row.get('Submitted By')), 'Reference': clean_text(row.get('Service Report e-SR')) or clean_text(row.get('PM Response ID')) or clean_text(row.get('Operation ID')), 'Status': clean_text(row.get('Submission Status')) or 'Submitted', 'Details': ' · '.join(detail_parts)})
    return rows
...
def _master_latest_service_row(master_row: pd.Series) -> list[dict[str, str]]:
    ...
    if not any([service_date, service_type, reference, technician]):
        return []
    return [{'Service Date': service_date, 'Service Type': service_type or 'Latest Service', 'Source': 'IB List Current Record', 'Technician': technician, 'Reference': reference, 'Status': 'Recorded', 'Details': 'Latest service information currently stored in the IB List.'}]

def _resolution_rows(issues: pd.DataFrame, resolutions: pd.DataFrame, serial: str) -> list[dict[str, str]]:
    ...
    if issue_matches.empty or resolutions.empty or 'Issue ID' not in resolutions.columns:
        return []
    issue_lookup = {clean_text(row.get('Issue ID')): row for _, row in issue_matches.iterrows() if clean_text(row.get('Issue ID'))}
    if not issue_lookup:
        return []
    matches = resolutions[resolutions['Issue ID'].astype(str).str.strip().isin(issue_lookup)].copy()
    rows: list[dict[str, str]] = []
    for _, row in matches.iterrows():
        issue_id = clean_text(row.get('Issue ID'))
        issue = issue_lookup.get(issue_id, pd.Series(dtype=object))
        details = []
        for label, column in [('Issue', 'Issue Type'), ('Action', 'Action Taken'), ('Parts', 'Parts Replaced'), ('Test', 'Test Performed'), ('Result', 'Test Result'), ('Notes', 'Resolution Notes')]:
            source_row = issue if column == 'Issue Type' else row
            value = clean_text(source_row.get(column))
            if value:
                details.append(f'{label}: {value}')
        rows.append({'Service Date': clean_text(row.get('Submitted At')) or clean_text(issue.get('Resolved At')) or clean_text(issue.get('Closed At')), 'Service Type': 'Issue Resolution', 'Source': 'Issue Management', 'Technician': clean_text(row.get('Submitted By')) or clean_text(issue.get('Resolved By')), 'Reference': clean_text(row.get('Submission ID')) or issue_id, 'Status': clean_text(row.get('Verification Result')) or clean_text(issue.get('Status')), 'Details': ' · '.join(details)})
    return rows
...
def _sort_service_history(dataframe: pd.DataFrame) -> pd.DataFrame:
    ...
    dataframe = dataframe.sort_values(['_sort_date', 'Source'], ascending=[False, True], na_position='last')
    return dataframe.drop(columns=['_sort_date']).reset_index(drop=True)

def build_service_history(master_row: pd.Series, serial: str, *, pm_responses_file: str | Path, issue_record_file: str | Path, resolution_file: str | Path, manual_service_file: str | Path=MANUAL_SERVICE_RECORDS_FILE) -> pd.DataFrame:
    ...
    history = history.drop_duplicates(subset=['Service Date', 'Service Type', 'Reference', 'Details'], keep='first')
    return _sort_service_history(history)

def load_unit_issues(serial: str, *, issue_record_file: str | Path) -> pd.DataFrame:
    issues = read_csv_safe(issue_record_file)
    matches = _matching_rows(issues, serial, ('Serial Number',))
    if matches.empty:
        return matches
    if 'Reported At' in matches.columns:
        parsed = pd.to_datetime(matches['Reported At'], dayfirst=True, errors='coerce')
        matches = matches.assign(_sort_date=parsed).sort_values('_sort_date', ascending=False, na_position='last').drop(columns=['_sort_date'])
    return matches.reset_index(drop=True)
UNIT_PROFILE_SEARCH_COLUMNS = ('Serial Number', 'Model', 'Location', 'Block / Locations', 'Street Name', 'Postal Code')

def filter_unit_profiles(dataframe: pd.DataFrame, keyword: str) -> pd.DataFrame:
    ...
    if search_text:
        ...
        for column in UNIT_PROFILE_SEARCH_COLUMNS:
            if column in filtered.columns:
                mask |= filtered[column].astype(str).str.casefold().str.contains(search_text, regex=False, na=False)
        filtered = filtered.loc[mask]
    if 'Serial Number' not in filtered.columns:
        return filtered.iloc[0:0].copy()
    filtered = filtered[filtered['Serial Number'].astype(str).str.strip().ne('')].copy()
