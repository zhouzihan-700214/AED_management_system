...
import pandas as pd
from services.aed_field_schema import DATE_FIELDS, JOB_TYPE_OPTIONS, REPAIRED_OPTIONS, REQUIRED_FIELDS, TABLE_EDITABLE_COLUMNS
...
def prepare_editor_dataframe(source_df: pd.DataFrame) -> pd.DataFrame:
    ...
    editor_df['Serial Number'] = editor_df['Serial Number'].astype(str).str.strip().str.upper()
    if editor_df['Serial Number'].eq('').any():
        raise ValueError('The editor contains an empty Serial Number.')
    duplicates = editor_df.loc[editor_df['Serial Number'].duplicated(keep=False), 'Serial Number'].unique()
    if len(duplicates):
        raise ValueError('Duplicate Serial Numbers found: ' + ', '.join(map(str, duplicates)))
...
def build_cell_changes(original_df: pd.DataFrame, edited_df: pd.DataFrame) -> list[dict[str, str]]:
    ...
    if original_serials != edited_serials:
        raise ValueError(f'Rows were unexpectedly added or removed in the editor. Missing: {sorted(original_serials - edited_serials)}; Unexpected: {sorted(edited_serials - original_serials)}')
    ...
    for serial in edited.index:
        for field in TABLE_EDITABLE_COLUMNS:
            ...
            if old_value == new_value:
                continue
            changes.append({'serial_number': serial, 'field': field, 'original_value': old_value, 'desired_value': new_value})
    return changes

def build_final_rows(original_df: pd.DataFrame, changes: list[dict[str, str]]) -> dict[str, dict[str, str]]:
    original = original_df.copy()
    ...
    for serial in affected:
        ...
        final_rows[serial] = {field: normalize_value(field, original.at[serial, field]) for field in original.columns if field != 'Serial Number'}
    for change in changes:
        final_rows[change['serial_number']][change['field']] = change['desired_value']
    return final_rows

def validate_table_changes(original_df: pd.DataFrame, changes: list[dict[str, str]]) -> tuple[list[str], list[str]]:
    errors: list[str] = []
    warnings: list[str] = []
    ...
    for serial, row in final_rows.items():
        ...
        if completed and next_pm:
            ...
            if next_date < completed_date:
                errors.append(f'{serial}: Next PM Date cannot be before PM Completed Date.')
        for field in ('Adult Pads Expiry Date', 'Pediatric Pads Expiry Date', 'Battery Expiry Date'):
...
def group_changes_for_repository(changes: list[dict[str, str]]) -> list[dict[str, Any]]:
    ...
    for change in changes:
        ...
        item = grouped.setdefault(serial, {'serial_number': serial, 'original_values': {}, 'desired_values': {}})
