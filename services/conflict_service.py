...
def detect_field_conflicts(original_values: Mapping[str, Any], current_values: Mapping[str, Any], desired_values: Mapping[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    for field, desired in desired_values.items():
        ...
        if current == desired_text:
            ...
            conflicts[field] = {'original': original, 'current': current, 'desired': desired_text}
    return {'safe_changes': safe_changes, 'already_applied': already_applied, 'conflicts': conflicts}
