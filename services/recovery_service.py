...
def recover_incomplete_transaction() -> dict[str, Any]:
    ...
    if step not in {'SOURCE_REPLACED', 'CACHE_SYNCED', 'RECOVERY_REQUIRED'}:
        ...
        record_transaction(**{'Operation ID': operation_id, 'Operation Type': transaction.get('operation_type', 'RECOVERY'), 'User': transaction.get('user', 'System'), 'Session ID': transaction.get('session_id', 'recovery'), 'Serial Numbers': '; '.join(transaction.get('serial_numbers', [])), 'Source Page': 'Startup Recovery', 'Started At': transaction.get('started_at', ''), 'Result': 'Recovered-Cleanup', 'Final Step': step, 'Message': message})
        return {'status': 'cleaned', 'message': message}
    metadata = {'operation_id': f'recovery-{operation_id}', 'operation_type': 'RECOVERY', 'user': 'System', 'session_id': 'startup-recovery', 'serial_numbers': transaction.get('serial_numbers', [])}
    try:
        ...
        record_transaction(**{'Operation ID': operation_id, 'Operation Type': transaction.get('operation_type', 'RECOVERY'), 'User': transaction.get('user', 'System'), 'Session ID': transaction.get('session_id', 'recovery'), 'Serial Numbers': '; '.join(transaction.get('serial_numbers', [])), 'Source Page': 'Startup Recovery', 'Started At': transaction.get('started_at', ''), 'Result': 'Recovered', 'Final Step': 'CACHE_SYNCED', 'Message': message})
