...
def _render_quick_aed_view(dataframe: pd.DataFrame) -> None:
    ...
    with action_col:
        if st.button("Open Master Table", width="stretch"):
            _navigate_management("AED Master Table")
...
def render_dashboard_unit_profiles(
    ...
    *,
    ...
    filtered = filtered.sort_values("Serial Number", kind="stable").reset_index(drop=True)
    ...
    if selected_serial not in options:
        selected_serial = ""
    if clean_text(st.session_state.get("unit_profile_selector")) not in options:
        st.session_state.pop("unit_profile_selector", None)
    ...
    with select_col:
        selected = st.selectbox(
            ...
            key="unit_profile_selector",
        )
    with master_col:
        st.markdown("<div style='height:1.72rem'></div>", unsafe_allow_html=True)
        if st.button("Open Master Table", width="stretch", key="profiles_open_master"):
            _navigate_management("AED Master Table")
    ...
    st.session_state["dashboard_profile_serial"] = selected_serial
    master_matches = dataframe[
        dataframe["Serial Number"].astype(str).str.strip().eq(selected_serial)
    ]
    if master_matches.empty:
...
def render_aed_management(
    ...
    """Compact boss overview with Unit Profiles as the primary work area."""
    del aed_data_file, history_file
    page_header(
        "AED Management",
        "See fleet status, open any AED electronic profile and focus on unresolved work.",
    ...
    # Unit Profiles deliberately replaces the old readiness area and is kept
    # above secondary management summaries so it is impossible to miss.
    render_dashboard_unit_profiles(dataframe)
