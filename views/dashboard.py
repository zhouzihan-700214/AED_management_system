...
from ui.dashboard_components import build_filtered_queue, render_activity_feed, render_control_header, render_global_toolbar, render_kpi_row, render_operational_summaries, render_quick_actions, render_selected_item_panel, render_source_health, render_work_queue
from views.aed_management import render_dashboard_unit_profiles

def render_dashboard(*, aed_data_file: str | Path, issue_record_file: str | Path, pm_response_file: str | Path, pm_plan_file: str | Path, aed_history_file: str | Path, issue_history_file: str | Path) -> None:
    """Boss-facing operations page with a direct Unit Profiles scope."""
    paths = DashboardPaths(aed_data=Path(aed_data_file), issue_records=Path(issue_record_file), pm_responses=Path(pm_response_file), pm_plan=Path(pm_plan_file), aed_history=Path(aed_history_file), issue_history=Path(issue_history_file))
    selected_period = st.session_state.get('dashboard_period')
    try:
        master_data = get_all_units()
        snapshot = build_dashboard_snapshot(paths=paths, selected_period=selected_period, aed_data=master_data)
    ...
    if filters['period'] != snapshot['period']:
        snapshot = build_dashboard_snapshot(paths=paths, selected_period=filters['period'], aed_data=master_data)
    if filters['view'] == 'Unit Profiles':
        render_dashboard_unit_profiles(snapshot['aed_data'], keyword=filters['keyword'], context_key='dashboard')
...
# Scope contract: filters["view"] == "Unit Profiles"
