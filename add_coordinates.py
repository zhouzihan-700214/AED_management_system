...
def main() -> None:
    ...
    for row_index, row in dataframe.iterrows():
        ...
        if result.success:
            success_count += 1
            print(f'Success: {serial_number or row_index} | {postal_code} | {result.latitude}, {result.longitude}')
        else:
            failure_count += 1
            print(f'Failed: {serial_number or row_index} | {postal_code} | {result.status}')
        time.sleep(0.3)
    atomic_write_csv(aed_service.ensure_columns(dataframe), OUTPUT_FILE, preferred_columns=aed_service.MASTER_COLUMNS)
